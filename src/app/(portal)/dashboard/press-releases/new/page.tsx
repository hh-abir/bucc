...
export default function NewPressReleasePage() {
  ...
  const handleSubmit = async (e: React.FormEvent, targetStatus: "draft" | "submit") => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(targetStatus === "draft" ? "Press Release saved as draft!" : isManager ? "Press Release published successfully!" : "Press Release submitted for review!");
    ...
    <div className="max-w-5xl mx-auto space-y-10 pb-20 pt-10">
      <div className="flex items-center justify-between border-b border-border pb-6">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-3xl font-serif font-bold tracking-tight text-foreground">Official Release</h1>
        </div>
        <div className="flex gap-4">
          ...
          >
            {isSubmitting ? "Submitting..." : isManager ? "Publish Release" : "Submit for Review"}
          </button>
        </div>
      </div>
